#!/usr/bin/env python
# ./polybar/scripts/polybar_onlinestatus.py
"""
...
class OnlineStatus:
    ...
    def __init__(self):
        ...
        self.cache_timeout = (
            self.args.cache_timeout if self.args.cache_timeout is not None else 5
        )
        ...
        if self.args.offline_color and search(
            "^#(?:[0-9a-fA-F]{3}){1,2}$", self.args.offline_color
        ):
            self.color["offline"] = f"%{{F{self.args.offline_color}}}"
        else:
            self.color["offline"] = "%{F#f00}"

        if self.args.online_color and search(
            "^#(?:[0-9a-fA-F]{3}){1,2}$", self.args.online_color
        ):
            self.color["online"] = f"%{{F{self.args.online_color}}}"
        else:
            self.color["online"] = "%{F#0f0}"

        self.boolean_mode = bool(self.args.boolean_mode)
        self.color_mode = bool(self.args.color_mode)
    ...
    def check_online(self):
        ...
        if "://" in self.url:
            ...
            try:
                return (
                    call(
                        ["ping", "-c", "1", "-W", f"{self.timeout}", f"{self.url}"],
                        stdout=DEVNULL,
                        stderr=DEVNULL,
                    )
                    == 0
        ...
        if self.boolean_mode is True:
            if self.color_mode is True:
                return f"{
                    ...
                    else self.color['offline']
                }{self.check_online()}{self.color['end']}"
            else:
            ...
            if self.color_mode is True:
                if self.check_online():
                    return (
                        f"{self.color['online']}{self.online_icon}{self.color['end']}"
                    )
                else:
                    return (
                        f"{self.color['offline']}{self.offline_icon}{self.color['end']}"
                    )
            else:
                if self.check_online():
